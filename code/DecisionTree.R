...
minsplit_values = c(default_minsplit/4, default_minsplit/2, default_minsplit, default_minsplit*2, default_minsplit*4)
...
for(i in 1:k) {
  ...
  for(i in 1:length(minsplit_values)){
    for(j in 1:length(minbucket_values)){
      ...
      tree = rpart(bug ~., dataset[train.index,], method="class", parms=list(split=args[1]), control=rpart.control(minsplit=minsplit_values[i],minbucket=minbucket_values[j],cp=0))
